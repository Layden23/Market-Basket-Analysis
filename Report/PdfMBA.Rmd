...
Retailers buy the following products togeher:

```{r, echo = FALSE}
results[c(1,2,3),2:4]
```

Retailers buy the following categories togeher:
...
Corporates buy the following products togeher:
...
We'll split the dataframe to obtain the rules for each diferent types of costumers. 
...
We will evaluate our rules by creating and average of the support, confidence and lift. First we'll rescale these three variables and then add them together.
```{r}
...
```
```{r}
corpro[,which(colnames(corpro) %in% c("lhs","Var.2","rhs","average"))]
```
...
```
```{r}
corcat[,which(colnames(corcat) %in% c("lhs","Var.2","rhs","average"))]
```
...
```
```{r}
retpro[,which(colnames(retpro) %in% c("lhs","Var.2","rhs","average"))]
```
...
```
```{r}
retcat[,which(colnames(retcat) %in% c("lhs","Var.2","rhs","average"))]
```
